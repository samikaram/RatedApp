...
def calculate_payment_amount(invoice):
    """Calculate payment amount using July 2nd proven logic"""
    # Try multiple payment detection methods from July 2nd success
    total_amount = float(invoice.get('total_amount', 0))
    
    # Method 1: Direct amount_paid field
    amount_paid = float(invoice.get('amount_paid', 0))
    if amount_paid > 0:
        return amount_paid
    
    # Method 2: Check if invoice status indicates payment
    status = invoice.get('status', '').lower()
    if status in ['paid', 'completed']:
        return total_amount
    
    # Method 3: Check payment_status field
    payment_status = invoice.get('payment_status', '').lower()
    if payment_status in ['paid', 'completed']:
        return total_amount
    
    # Method 4: Check for payment records/transactions
    if invoice.get('payments') and len(invoice.get('payments', [])) > 0:
        return total_amount
    
    # Method 5: Outstanding amount logic (if outstanding is 0, it's paid)
    outstanding = float(invoice.get('outstanding_amount', total_amount))
...
def main():
    print("🎯 REVENUE-FIXED PATIENT BEHAVIOR EXTRACTION")
    print("="*70)
    print("🔧 Using July 2nd proven payment detection logic")
    ...
    try:
        ...
        # Step 4: Calculate behavior metrics with FIXED REVENUE LOGIC
        print(f"\n🎯 STEP 4: Calculating behavior metrics with fixed revenue logic...")
        results = []
        total_clinic_revenue = 0
        total_clinic_invoiced = 0
        ...
        for patient_id, data in patient_behaviors.items():
            ...
            # FIXED FINANCIAL METRICS using July 2nd proven logic
            total_invoiced = sum(float(inv.get('total_amount', 0)) for inv in invoices)
            total_paid = sum(calculate_payment_amount(inv) for inv in invoices)
            
            # Add to clinic totals
            total_clinic_invoiced += total_invoiced
            total_clinic_revenue += total_paid
        ...
        if results:
            ...
            print(f"   💰 Total invoiced: ${total_clinic_invoiced:.2f}")
            print(f"   💵 Total revenue: ${total_clinic_revenue:.2f}")
            ...
            # Show top performers by activity
        ...
        # Step 6: Save results
        filename = f"revenue_fixed_patient_behaviors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(filename, 'w') as f:
            json.dump({
                ...
                'clinic_statistics': {
                    ...
                    'avg_payment_rate': avg_payment_rate if results else 0,
                    'total_invoiced': total_clinic_invoiced,
                    'total_revenue': total_clinic_revenue
                },
                'patient_behaviors': results
            }, f, indent=2, default=str)
        
        print(f"\n💾 Complete results saved to: {filename}")
        print(f"✅ REVENUE-FIXED EXTRACTION COMPLETE - Ready for Django integration!")
