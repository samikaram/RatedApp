...
class PatientSearchView(View):
    ...
    def search_by_name(self, name):
        """Complete fuzzy search - progressive filtering on both first and last names"""
        search_term = name.strip().lower()
        
        if len(search_term) < 1:
            return []
        
        # Search first names with prefix matching
        url = f"{BASE_URL}/patients"
        params = {'q[]': f'first_name:~{search_term}'}
        response = requests.get(url, headers=HEADERS, params=params)
        response.raise_for_status()
        data = response.json()
        first_name_patients = data.get('patients', [])
        
        # Search last names with prefix matching
        params = {'q[]': f'last_name:~{search_term}'}
        response = requests.get(url, headers=HEADERS, params=params)
        response.raise_for_status()
        data = response.json()
        last_name_patients = data.get('patients', [])
        
        # For multi-word searches, try intelligent splitting
        name_parts = search_term.split()
        combined_search_patients = []
        
        if len(name_parts) > 1:
            # Multi-word search: first word in first name, last word in last name
            first_term = name_parts[0]
            last_term = name_parts[-1]
            
            # Search: first_name starts with first term AND last_name starts with last term
            params = {
                'q[]': [
                    f'first_name:~{first_term}',
                    f'last_name:~{last_term}'
                ]
            }
            response = requests.get(url, headers=HEADERS, params=params)
            if response.status_code == 200:
                data = response.json()
                combined_search_patients = data.get('patients', [])
        
        # Combine all results and remove duplicates
        all_patients = first_name_patients + last_name_patients + combined_search_patients
        unique_patients = []
        seen_ids = set()
        ...
        print(f"🔍 COMPLETE SEARCH: '{name}' → {len(first_name_patients)} first + {len(last_name_patients)} last + {len(combined_search_patients)} combined = {len(unique_patients)} unique")
        ...
        for patient in unique_patients:
            ...
            formatted_patients.append({
                ...
                'last_name': last_name,
                'phone_numbers': self.get_phone_numbers(patient)
            })
        
        # Sort results: prefix matches first, then others
        search_term_lower = name.strip().lower()
        ...
        def sort_key(patient):
            ...
            # Priority 1: Full name starts with search term
            if full_name.startswith(search_term_lower):
                return (0, full_name)
            # Priority 2: First name starts with search term
            elif first_name.startswith(search_term_lower):
                return (1, first_name)
            # Priority 3: Last name starts with search term
            elif last_name.startswith(search_term_lower):
                return (2, last_name)
            # Priority 4: First name contains search term
            elif search_term_lower in first_name:
                return (3, first_name)
            # Priority 5: Last name contains search term
            elif search_term_lower in last_name:
                return (4, last_name)
            # Priority 6: Everything else
            else:
                return (5, full_name)
        
        formatted_patients.sort(key=sort_key)
        print(f"📊 SORTED RESULTS: {len(formatted_patients)} patients ordered by relevance")
...
def unified_dashboard(request):
    ...
    # Load bracket data for template (fixes refresh issue)
    age_brackets = []
    spend_brackets = []
