            ...
            # Bulk bracket operations for preset application
                ...
                if points_value:
                    try:
                        ...
                        if 1 <= points_int <= 100:
                            # DISABLED: active_config.points_per_unpaid_invoice = points_int # Prevents Factory Settings overwrite
                            # DISABLED: active_config.save() # Prevents Factory Settings overwrite
                            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                                return JsonResponse({"success": True})
                        else:
                            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            ...
            # Handle DNA points update
                ...
                if points_value:
                    try:
                        points_int = int(points_value)
                        if 1 <= points_int <= 100:
                            # DISABLED: active_config.points_per_dna = points_int # Prevents Factory Settings overwrite
                            # DISABLED: active_config.save() # Prevents Factory Settings overwrite
                            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                                return JsonResponse({"success": True})
                        else:
            ...
            # Handle cancellations points update
                ...
                if points_value:
                    try:
                        points_int = int(points_value)
                        if 1 <= points_int <= 100:
                            # DISABLED: active_config.points_per_cancellation = points_int # Prevents Factory Settings overwrite
                            # DISABLED: active_config.save() # Prevents Factory Settings overwrite
                            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                                return JsonResponse({"success": True})
                        else:
            ...
            # Handle consecutive attendance points update
                ...
                if points_value:
                    try:
                        points_int = int(points_value)
                        if 1 <= points_int <= 100:
                            # DISABLED: active_config.points_per_consecutive_attendance = points_int # Prevents Factory Settings overwrite
                            # DISABLED: active_config.save() # Prevents Factory Settings overwrite
                            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                                return JsonResponse({"success": True})
                        else:
                ...
                try:
                    ...
                    # Get preset and check if it's currently active
                    preset = ScoringConfiguration.objects.get(id=preset_id)
                    preset_name = preset.name
                    was_active = preset.is_active
                    
                    # If active preset is being deleted, find fallback
                    fallback_preset = None
                    if was_active:
                        fallback_preset = ScoringConfiguration.objects.exclude(id=preset_id).first()
                        if fallback_preset:
                            fallback_preset.is_active = True
                            fallback_preset.save()
                    
                    # Delete the preset
                    preset.delete()
                    
                    # Enhanced JSON response with fallback data
                    return JsonResponse({
                        "success": True, 
                        "message": f"Preset {preset_name} deleted successfully",
                        "was_active_preset_deleted": was_active,
                        "deleted_preset_name": preset_name,
                        "fallback_preset": {
                            "id": fallback_preset.id if fallback_preset else None,
                            "name": fallback_preset.name if fallback_preset else None,
                        } if fallback_preset else None
                    })
                    return JsonResponse({"success": False, "error": "Preset not found"})
                except Exception as e:
                    return JsonResponse({"success": False, "error": str(e)})
                ...
                try:
                    ...
                    # Create new ScoringConfiguration with current form values
                    new_preset = ScoringConfiguration.objects.create(
                        name=preset_name,
                        description=preset_description,
                        is_active=False,  # New presets are not active by default
                        
                        # Copy current slider values from form
                        future_appointments_weight=int(request.POST.get('future_appointments_weight', 20)),
                        age_demographics_weight=int(request.POST.get('age_demographics_weight', 10)),
                        yearly_spend_weight=int(request.POST.get('yearly_spend_weight', 25)),
                        consecutive_attendance_weight=int(request.POST.get('consecutive_attendance_weight', 30)),
                        likability_weight=int(request.POST.get('likability_weight', 20)),
                        cancellations_weight=int(request.POST.get('cancellations_weight', 20)),
                        dna_weight=int(request.POST.get('dna_weight', 15)),
                        unpaid_invoices_weight=int(request.POST.get('unpaid_invoices_weight', 50)),
                        open_dna_invoice_weight=int(request.POST.get('open_dna_invoice_weight', 100)),
                        
                        # Copy points per occurrence values
                    ...
                    # Return preset details for frontend dropdown update
                    return JsonResponse({
                        "success": True,
                    ...
                    return JsonResponse({"success": False, "error": f"Error creating preset: {str(e)}"})
                ...
                # Handle inline edit actions (do not update database)
...
def create_preset(request):
    """
    Create a new preset by copying current active configuration values.
    
    Workflow:
    1. Validate name (required, max 50 chars)
    2. Auto-save current screen state to active config
    3. Create new preset copying from active config
    4. Return success with preset details
    """
    if request.method == 'POST':
        try:
            import json
            ...
            if ScoringConfiguration.objects.filter(name=name).exists():
                return JsonResponse({'success': False, 'error': f'Preset name "{name}" already exists'})
            
            # Get active configuration (source for copying)
            active_config = ScoringConfiguration.objects.filter(is_active=True).first()
            if not active_config:
                return JsonResponse({'success': False, 'error': 'No active configuration found'})
            
            # Create new preset by copying active config
            new_preset = ScoringConfiguration.objects.create(
                name=name,
                description=description,
                is_active=False,  # New presets are not active by default
                
                # Copy all weight values from active config
                ...
                # Copy all points values from active config
                            # DISABLED: points_per_consecutive_attendance=active_config.points_per_consecutive_attendance, # Prevents Factory Settings overwrite
                            # DISABLED: points_per_cancellation=active_config.points_per_cancellation, # Prevents Factory Settings overwrite
                            # DISABLED: points_per_dna=active_config.points_per_dna, # Prevents Factory Settings overwrite
                            # DISABLED: points_per_unpaid_invoice=active_config.points_per_unpaid_invoice # Prevents Factory Settings overwrite
            )
...
def patient_list(request):
    """
    Simple patient list view - placeholder for future implementation
    """
    from django.http import HttpResponse
    return HttpResponse('<h1>Patient List</h1><p>This feature will be implemented in the future.</p>')

def patient_detail(request, patient_id):
    """
    Simple patient detail view - placeholder for future implementation
    """
    from django.http import HttpResponse
    return HttpResponse(f'<h1>Patient Detail</h1><p>Patient ID: {patient_id}</p><p>This feature will be implemented in the future.</p>')
