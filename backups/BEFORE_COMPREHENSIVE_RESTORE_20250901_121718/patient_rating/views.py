...
from patient_rating.models import RatedAppSettings
import requests
import base64
import time
...
class PatientSearchView(View):
    ...
    def search_by_name(self, name):
        ...
        def sort_key(patient):
            ...
            if first_name.startswith(search_term_lower):
                return (0, first_name)
...
def update_clinic_settings(request):
    try:
        data = json.loads(request.body)
        settings, created = RatedAppSettings.objects.get_or_create(
            pk=1,
            defaults={
                'clinic_name': 'Sports Medicine Clinic',
                'clinic_location': 'Parramatta',
                'clinic_timezone': 'Australia/Sydney'
            }
        )
        
        settings.clinic_name = data.get('clinic_name', settings.clinic_name)
        settings.clinic_location = data.get('clinic_location', settings.clinic_location)
        settings.clinic_timezone = data.get('clinic_timezone', settings.clinic_timezone)
